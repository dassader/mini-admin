...
Локальное mobile-first React + TypeScript приложение для диагностики и управления Mini Hardware Bus и ZigBee-сетью прямо из Chrome через Web Serial.
...
Web Serial требует Chrome/Edge и secure context (`localhost` или HTTPS). Один USB serial port может принадлежать только одному процессу. Если плата уже открыта `mini-service-bus-mcp`, сначала отключите transport в MCP, затем выбирайте устройство в браузере.
